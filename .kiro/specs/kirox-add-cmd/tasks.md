...
- [ ] 8.8 インタラクティブモード開始前のメタデータ読み込みタイミングを修正（BUG FIX）
  ...
  - **期待動作**:
    ...
    - `--track`オプションがfalseの場合は、メタデータ操作を完全にスキップする
  - **根本原因**:
  ...
  - **影響範囲**:
    ...
    - `--track`オプションfalse時のメタデータスキップ処理（追加が必要）
  - **修正内容**:
    ...
    - **アプローチB（代替案）**: メタデータ読み込みを常にインタラクティブモード後に移動
      - すべてのモード（interactive/non-interactive）で、メタデータ読み込みを引数確定後に統一
      - リポジトリ提案機能は無効化される（既存メタデータを参照できないため）
  - **テスト確認項目**:
    - [ ] Non-interactiveモードでメタデータが即座に読み込まれることを確認
    - [ ] Interactiveモードでoutput確定後にメタデータが読み込まれることを確認
    - [ ] Interactive mode開始時に不正なメタデータパスでログが出力されないことを確認
    - [ ] `--track=false`時にメタデータ操作が完全にスキップされることを確認
